...
import { getMonthlyClose, getIEXData } from './actions/index';
...
class MonthlyPage extends Component {
    ...
    render() {
        ...
        var dayPriceObj = monthTicker["Monthly Time Series"]              //separates closing prices from dayTicker assigns it to dayPriceObj
        if (dayPriceObj) {                                              
            getPriceSet(dayPriceObj)                                    //returns a array of 12 objects of closing prices
        }
        var dayInfoObj = monthTicker["Meta Data"]                         //separates meta data from dayTicker
        if (dayInfoObj) {                                               //getCoInfo normalizes dayInfoObj
            getCoInfo(dayInfoObj)                                       //returns a array of 1 object, company ticker and date
        }
        var dayPriceObj = monthTicker["Monthly Time Series"]              //separates closing pices form dayTicker and assigns it to dayPriceObj
        if (dayPriceObj) {                                              
        ...
        return (
            <div className="page-container">
                <Navbar />
                <h2 className="page-header">Long Term Snapshot</h2>
...
export default connect(mapStateToProps, mapPropsToDispatch)(MonthlyPage);
